package org.example.onlinepossystem.pizza.repository;
...
public interface PizzaReadRepository extends JpaRepository<Pizza, Integer> {
    ...
    @Query("""
            select new org.example.onlinepossystem.pizza.dto.PizzaSizePriceRow(
                p.id,
                p.name,
                s.cm,
                bpp.price,
    ...
    @Query("""
            select new org.example.onlinepossystem.pizza.dto.ToppingRow(
                i.id,
                i.name,
                pdi.sortOrder,
                bep.price
