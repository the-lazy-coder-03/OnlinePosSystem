...
CREATE FUNCTION app_security.setting_id(setting_name text) RETURNS bigint
...
CREATE FUNCTION app_security.actor_role() RETURNS text
...
BEGIN
    ...
    FOREACH table_name IN ARRAY ARRAY[
        ...
        'staff', 'password_reset_tokens', 'orders'
    ] LOOP
...
CREATE FUNCTION app_security.guard_account_update() RETURNS trigger
...
CREATE FUNCTION app_security.guard_order_relationship() RETURNS trigger
LANGUAGE plpgsql SET search_path = pg_catalog, pg_temp AS $$
...
CREATE FUNCTION app_security.account_context(account_id bigint) RETURNS TABLE(access_level smallint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $$
    SELECT c.access_level FROM public.customers c WHERE c.id = account_id
$$;
CREATE FUNCTION app_security.login_credentials(identifier text)
...
$$;
CREATE FUNCTION app_security.account_exists(email_value text, phone_value text) RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $$
    SELECT EXISTS (SELECT 1 FROM public.customers c WHERE c.email = email_value OR c.phone1 = phone_value)
$$;
CREATE FUNCTION app_security.register_customer(
    ...
    RETURNING id
$$;
CREATE FUNCTION app_security.create_reset(email_value text, hash_value text, expiry timestamp) RETURNS text
...
BEGIN
    ...
    RETURN recipient;
END $$;
CREATE FUNCTION app_security.cancel_reset(hash_value text) RETURNS void
LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $$
    DELETE FROM public.password_reset_tokens WHERE token_hash = hash_value AND NOT used
$$;
CREATE FUNCTION app_security.consume_reset(hash_value text, password_value text) RETURNS boolean
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $$
