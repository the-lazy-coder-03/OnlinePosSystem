...
-- =========================================================
-- 3.2) STAFF
--      Staff PIN and branch-code authentication remains separate from
...
CREATE TABLE IF NOT EXISTS order_menu_item (
                                               ...
                                               unit_price_at_time DOUBLE PRECISION NOT NULL CHECK (unit_price_at_time >= 0),
...
CREATE TABLE IF NOT EXISTS order_menu_item_extra (
                                                     ...
                                                     name                     TEXT NOT NULL,
                                                     qty                      INT NOT NULL DEFAULT 1 CHECK (qty > 0),
                                                     unit_price_at_time       DOUBLE PRECISION NOT NULL DEFAULT 0 CHECK (unit_price_at_time >= 0)
);
...
CREATE TABLE IF NOT EXISTS order_pizza_item (
                                                ...
                                                pizza_size_id        INT NOT NULL REFERENCES pizza_size(pizza_size_id) ON UPDATE RESTRICT ON DELETE RESTRICT,
                                                qty                  INT NOT NULL DEFAULT 1 CHECK (qty > 0),
                                                base_price_at_time   DOUBLE PRECISION NOT NULL CHECK (base_price_at_time >= 0),
                                                notes                TEXT
);
...
-- the admin catalog. Update those rows by their natural key before inserting
-- anything that is still missing.
LOCK TABLE menu_item IN SHARE ROW EXCLUSIVE MODE;

WITH dessert_seed(preferred_id, category_id, name, description) AS (
    VALUES
        (1101, 11, 'Magnum', 'Ola ice cream'),
        (1102, 11, 'Cornetto', 'Ola ice cream'),
        (1103, 11, 'Paddle Pop', 'Ola ice cream')
)
...
SET description = seed.description,
    ...
    is_2l = FALSE,
    active = TRUE
FROM dessert_seed AS seed
WHERE item.category_id = seed.category_id
  AND item.name = seed.name;

-- Prefer the stable seed IDs on a fresh database. When a preferred ID already
-- belongs to another catalog row, allocate a new ID above the current range.
WITH dessert_seed(preferred_id, category_id, name, description) AS (
    VALUES
        (1101, 11, 'Magnum', 'Ola ice cream'),
        (1102, 11, 'Cornetto', 'Ola ice cream'),
...
missing_dessert AS (
    SELECT seed.*,
           row_number() OVER (ORDER BY seed.preferred_id) AS fallback_offset
    FROM dessert_seed AS seed
    WHERE NOT EXISTS (
        SELECT 1
        FROM menu_item AS existing
        WHERE existing.category_id = seed.category_id
          AND existing.name = seed.name
    )
),
id_floor AS (
    SELECT GREATEST(COALESCE(MAX(id), 0), 1103) AS value
    FROM menu_item
)
INSERT INTO menu_item (id, category_id, name, description, is_300ml, is_2l)
SELECT CASE
           WHEN NOT EXISTS (
               SELECT 1
               FROM menu_item AS occupied
               WHERE occupied.id = missing.preferred_id
           ) THEN missing.preferred_id
           ELSE id_floor.value + missing.fallback_offset
       END,
       missing.category_id,
       missing.name,
       missing.description,
       FALSE,
       FALSE
FROM missing_dessert AS missing
CROSS JOIN id_floor
